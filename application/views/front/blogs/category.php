				...
				<div class="about-right">
					<nav aria-label="breadcrumb">
					  	<ol class="breadcrumb">
                    		...
                    		<?php if(isset($month)){?>
								<li class="breadcrumb-item active">Archive- <?=$this->commonmod_model->getMonthName($month)?> <?=@$year?> </li>
								<?php } else if(isset($searckkey)){ ?>
								<li class="breadcrumb-item active">Search - <?=@$searckkey?></li>
								<?php }else{?>
								<li class="breadcrumb-item active">Category - <?=@$CategoryData['category_name'];?></li>
							<?php } ?>
					  	</ol>
					</nav>
				</div>
								...
								<ul class="media-list">
									<li class="media animated out" data-animation="fadeInLeft" data-delay="0">
										<div class="col-md-12 col-sm-12 col-xs-12">
											...
											?>
											<a href="<?=BASE_URL?>blog/<?=$url_title;?>"> <img src="<?= BASE_URL.$src?>" class="media-object" alt="image" style="max-width:100%;max-height:100%;"></a>
										
											<div class="media-body">
												<header class="post-heading">
													<div class="date-tag"><span><?=$day?></span><?=$month?></div>
													<h4 class="media-heading headingfont"><a href="<?=BASE_URL?>blog/<?=$url_title;?>"><?=$BlogData['title']?></a></h4>
												</header>
												<div class="clearfix"></div>
												<p>
													<!--<?=substr($BlogData['description'],0,200)?>-->
													<?=substr(strip_tags($BlogData['description']), 0, 200)?>
													<a href="<?=BASE_URL?>blog/<?=$url_title;?>">[Read More]</a>
												</p>
											</div>
										</div>
									</li>
			...
			<!-- <div class="col-lg-3">
                ...
                <div class="widget widget_categories animated fadeInUp in" data-animation="fadeInUp" data-delay="0">
                    ...
                    <ul>
						...
						?>
                        <li>
							<a href="<?php if($BlogCatData['id'] ==""){echo '#';}else{ echo BASE_URL?>category/<?=$cat_url_title; }?>"><?=$BlogCatData['category_name']?></a>
