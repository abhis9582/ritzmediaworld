    ...
    {       
        ...
        if ($numrows == 1) {			
            ...
            $data = $query->result_array();
			 ...
			 $this->CI->session->set_userdata('bh_admin_name', $data[0]["username"]);
	 ...
	 public function check_front_login()    {
      
	    $user_id = $this->CI->session->userdata('bh_front_user_id');
	    if ($user_id=='') { 
	    $msg = array('login' => '<p>You must be logged in to access user dashboard area.</p>');           
        redirect(BASE_URL.'login?msg=login',$msg);

        }
        
    } 
	function process_front_login($email, $password)
    {       
        ...
        if ($numrows == 1) {			
            ...
            $data = $query->result_array();
		     ...
		     if($data[0]["user_type"]==1){ $usertype = 'Agent' ; } else { $usertype = 'User'; }
             $this->CI->session->set_userdata('front_logged_in', '1');			
            		
			 $this->CI->session->set_userdata('bh_front_user_id', $data[0]["user_id"]);
			 $this->CI->session->set_userdata('bh_front_email_id', $data[0]["email_id"]);
			 $this->CI->session->set_userdata('bh_front_username', $data[0]["first_name"]." ".$data[0]["last_name"]);
			 $this->CI->session->set_userdata('bh_front_mobile', $data[0]["mobile"]);
			 $this->CI->session->set_userdata('bh_front_phone', $data[0]["phone"]);
			 $this->CI->session->set_userdata('bh_front_usertype', $usertype);
		
			

        } else {
            $this->CI->session->set_userdata('front_logged_in', '');
