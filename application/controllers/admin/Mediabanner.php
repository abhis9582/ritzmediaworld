...
class Mediabanner extends CI_Controller {
 ...
 $this->load->library('image');   		 
	...
	{
		$data['Mediabanner'] =  $this->mediabanner_model->getALLImage();
		$this->load->view('admin/mediabanner/index',$data);
	}
	...
	public function add()
	{
		 ...
		 {	
	      ...
	      $config = array( 	           
			...
			$this->load->view('admin/mediabanner/add'); 
		...
		('./webroot/images/original/','./webroot/images/gallery/','3048','1800','1000','image_name',$error_view_url);
			...
			redirect(BASE_URL.'admin/mediabanner');
			}
		 }
		$this->load->view('admin/mediabanner/add');
	...
	{
		$data['GalleryData'] =  $this->mediabanner_model->GetImageByID($id);
		   if($id==""){  
		 ...
		 {	
	      $config = array( 	           
			...
			$this->load->view('admin/mediabanner/edit',$data); 
...
$editdata  = $this->mediabanner_model->GetImageByID($id);
